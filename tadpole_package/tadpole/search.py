import itertools
import RNA
from tadpole.rna_structures import get_pairing_dict, constrained_mfe, get_base_pairs
from tadpole.rna_mutation import mutate_sequence, mutate_with_complement
...
def run_linker_search(rna1, rna3, struct1, constraint,
                      ...
                      use_mutations=False, mfe_delta=0,
    ...
    """
    # Initialize counters for discarded linkers based on different criteria
    discarded_counts = {
        "pairings_exceed": 0,
    ...
    # Store a few examples of discarded linkers for the final report
    discarded_examples = {
        "pairings_exceed": [],
    ...
    # Main loop to iterate through each specified linker length
    for linker_length in linker_lengths:
        # Generate all possible RNA sequences (A, U, G, C) for the current linker length (brute-force)
        ...
        count = 0
        # Iterate through each generated linker sequence
        for linker in all_linkers:
            
            # Check if a stop request has been made from the UI (e.g., a stop button click)
            if check_stop_func:
                val = check_stop_func()
                if val:
            ...
            # Log progress every 1000 linkers
            if (count % 1000 == 0):
                log_func(f'Tryed {count} linkers')
            count = count + 1
            
            # Skip this linker if it has already been processed
            if linker in linkers_tried:
                continue
            linkers_tried.add(linker)

            # Concatenate the sequences to form the full RNA construct
            seq_full = rna1 + linker + rna3
            # Fold the full sequence to get its unconstrained (OFF) structure and MFE
            struct_full, mfe_1 = RNA.fold(seq_full)

            # Get pairing dictionaries for the original and new structures
            pairing1 = get_pairing_dict(struct1)
            pairing_test = get_pairing_dict(struct_full)

            # Check if the 'watched_positions' have changed their pairing status
            changed = all(pairing1.get(pos) != pairing_test.get(pos) for pos in watched_positions)
            # Count the number of pairings between rna1 and rna3 in the full unconstrained structure
            pairings_count = count_rna1_rna3_pairings(struct_full, rna1, rna3, linker)
            
            # Filter 1: Discard if rna1-rna3 pairings exceed the maximum
            if pairings_count > max_pairings_rna1_rna3:
                ...
                continue
            # Filter 2: Discard if watched positions did not change as required
            if not changed:
            ...
            # Calculate the constrained (ON) structure and MFE
            struct_constr, mfe_2 = constrained_mfe(seq_full, constraint)
            # Check how many base pairs in the RNA1 part changed from the original struct1 to the constrained structure
            changes_rna1 = check_rna1_structure_preserved(rna1, linker, rna3, struct_constr, struct1)
            
            # Filter 3: Discard if the RNA1 substructure changes too much in the ON state
            ...
            # If mutations are not enabled, apply the final MFE filter
            if not use_mutations:
                if mfe_1 >= mfe_2 - mfe_delta:
                    ...
                    continue
                # If all criteria are met, save the result
            ...
            for n_mut in range(0, num_mut):
                for rna1_mut, mut1_info in mutate_sequence(rna1, mutable_rna1, n_mut):
                    ...
                    # Check a different set of criteria for mutated sequences
                    changed = all(pairing1.get(pos) == pairing_full.get(pos) for pos in watched_positions)
                    pairings_count = count_rna1_rna3_pairings(struct_full, rna1, rna3, linker)

                    # Filter out invalid mutated designs
                    if not changed or pairings_count > max_pairings_rna1_rna3:
                        continue

                    # Evaluate constrained structure and MFE
                    struct_constr, mfe_2 = constrained_mfe(seq_full, constraint)
                    changes_rna1 = check_rna1_structure_preserved(rna1, linker, rna3, struct_constr, struct1)
                    if changes_rna1 > max_structure_changes:
                        continue

                    # If all criteria are met, save the valid mutated linker
                    if mfe_1 < mfe_2 - mfe_delta:
                        ...
                        # Plot and save the structures for the mutated design
                        save_and_plot_structures(seq_full, struct_full, struct_constr, mutated_rna1, linker, rna3, mut1_info, mfe_1, mfe_2)
                        count_found += 1
                        break  # Stop trying more mutations for this linker
        
        # Check again for a stop request after a full linker length has been processed
        if stop_requested:
            break
        
    print('strart clustering')

    # Post-search clustering and visualization for all found results
    if len(results) > 1:
        ...
        # Calculate the RMSD matrix for structural similarity
        ...
        # Organize and summarize the results based on clusters
        ...
        # Compute and visualize other metrics
    ...
    # Add a warning if a large percentage of linkers failed due to the MFE criteria
